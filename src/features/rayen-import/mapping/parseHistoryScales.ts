 ...
 * Archived campos are dropped (superseded values); an event whose scale has no non-archived campos is
 * skipped. The result is day-agnostic history — use `evaluationScalesForCensusDay` (from
...
export const parseHistoryScales = (rawEvents: unknown): EvaluationScale[] => {
  ...
  for (const event of events) {
    ...
    // Group this event's live campos by form name (defensive: an event may bundle both scales).
    const byForm = new Map<string, RawResumeCampo[]>();
    for (const campo of campos) {
      if (campo?.ARCHIVED === true) continue;
