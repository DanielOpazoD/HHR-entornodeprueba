...
export interface PreviousDayProbes {
  ...
  /** Whether `day` already holds this exact episode (or the RUT for legacy evidence). */
  alreadyDischarged: (day: string, rut: string, encounterId?: string) => boolean;
}
...
export const planPreviousDayEdits = (
  ...
  for (const candidate of candidates) {
    ...
    if (probes.alreadyDischarged(day, candidate.rut, candidate.encounterId)) continue;
