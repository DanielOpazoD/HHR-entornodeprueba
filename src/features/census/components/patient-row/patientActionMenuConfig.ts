import { Ambulance, ArrowRightLeft, Copy, LogOut, Scissors, Trash2, type LucideIcon } from 'lucide-react';

export type PatientRowAction = 'clear' | 'copy' | 'move' | 'discharge' | 'transfer' | 'cma';

interface UtilityActionConfig {
    action: Extract<PatientRowAction, 'clear' | 'copy' | 'move'>;
    label: string;
    title: string;
    icon: LucideIcon;
...
interface ClinicalActionConfig {
    action: Extract<PatientRowAction, 'discharge' | 'transfer' | 'cma'>;
    label: string;
    icon: LucideIcon;
    iconClassName: string;
}
