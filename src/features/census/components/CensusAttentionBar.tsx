 ...
 * how many occupied rows need attention right now, broken down by kind (signo vital alterado,
 * escala de riesgo por reaplicar, aislamiento). Reuses the SAME per-row acuity the left rail uses,
 * so the bar and the rails always agree. Renders nothing when the census is all-clear.
 */

import React, { useMemo } from 'react';
import { Activity, AlarmClock, Biohazard, TriangleAlert } from 'lucide-react';
import {
...
export const CensusAttentionBar: React.FC<CensusAttentionBarProps> = ({ beds, censusIsoDay }) => {
  ...
  if (summary.vital > 0) {
    items.push(
      <CountItem
        key="vital"
        icon={<Activity size={12} strokeWidth={2.5} aria-hidden />}
        label={plural(summary.vital, 'signo alterado', 'signos alterados')}
      />
    );
  }
  if (summary.scale > 0) {
