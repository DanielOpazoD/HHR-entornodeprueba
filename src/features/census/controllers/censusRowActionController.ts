...
import { DEFAULT_EVACUATION_METHOD, DEFAULT_RECEIVING_CENTER } from '@/constants';
import { PatientData, CMAData } from '@/types';
...
export type RowActionErrorCode =
    | 'ACTIONS_LOCKED'
    | 'PATIENT_NAME_REQUIRED';

export type RowActionError = ControllerError<RowActionErrorCode>;

export type RowActionResult = ControllerResult<RowActionCommand, RowActionErrorCode, RowActionError>;
...
export const resolveRowActionCommand = ({
    ...
    if (!stabilityRules.canPerformActions) {
        return failWithCode('ACTIONS_LOCKED', stabilityRules.lockReason || 'Este registro está bloqueado para ediciones.');
    }
