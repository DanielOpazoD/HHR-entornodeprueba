...
export const TransferReceivingSection: React.FC<TransferReceivingSectionProps> = ({
  ...
  const receivingCenterId = useId();
  const receivingCenterOtherId = useId();

  return (
    <div className="space-y-4 pt-2">
      <div className="space-y-1.5">
        {isCenterLocked ? (
          <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">
            Centro que Recibe
          </span>
        ) : (
          <label
            htmlFor={receivingCenterId}
            className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1"
          >
            Centro que Recibe
          </label>
        )}
      ...
      {!isCenterLocked &&
        (receivingCenter === RECEIVING_CENTER_OTHER ||
          ...
          <div className="animate-fade-in space-y-1.5 border-l-2 border-blue-50 pl-4">
            <label
              htmlFor={receivingCenterOtherId}
              className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1"
            >
              Especifique Centro
            </label>
            <input
              id={receivingCenterOtherId}
              type="text"
              className={clsx(
