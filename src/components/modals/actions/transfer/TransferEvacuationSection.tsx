...
export const TransferEvacuationSection: React.FC<TransferEvacuationSectionProps> = ({
  ...
  const evacuationMethodId = useId();
  const escortSelectId = useId();
  const evacuationMethodOtherId = useId();

  return (
    <div className="grid grid-cols-1 gap-4">
      <div className="space-y-1.5">
        <label
          htmlFor={evacuationMethodId}
          className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1"
        >
          Medio de Evacuación
        </label>
        <select
          id={evacuationMethodId}
      ...
      {evacuationMethod === EVACUATION_METHOD_OTHER && (
        <div className="animate-fade-in space-y-1.5 border-l-2 border-blue-50 pl-4">
          <label
            htmlFor={evacuationMethodOtherId}
            className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1"
          >
            Especifique Método
          </label>
          <input
            id={evacuationMethodOtherId}
            type="text"
            className={clsx(
