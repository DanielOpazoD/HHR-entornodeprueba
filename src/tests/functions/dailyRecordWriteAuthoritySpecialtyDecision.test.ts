...
describe('specialty decision through the real clinical authority callable', () => {
  ...
  it('commits a manual selection and one audit event in the same transaction', async () => {
    ...
    const { admin, docRef, transaction, update, set } = createAdminMock({
      remoteData: remote,
      specialtyPolicyData,
    });
    ...
    expect(update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        'beds.R1.specialtyAssignment': expect.objectContaining({
          source: 'manual',
    ...
    expect(set).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ decisionId: 'new-mutation' })
    );
  ...
  it('does not erase an occupied legacy specialty during an unrelated partial write', async () => {
    ...
    const { admin, docRef, update } = createAdminMock({
      remoteData: remote,
    ...
    const response = await api.patchDailyRecordWithClinicalAuthority.run(
      {
        date: base.date,
        patch: { 'beds.R1.age': '42' },
        syncContract: {
          mutationId: 'legacy-age-edit',
          baseRevision: 2,
    ...
    expect(response.success).toBe(true);
    const write = update.mock.calls.find(([reference]) => reference === docRef)?.[1] as Record<
      string,
      unknown
    >;
  ...
  it('retains a confirmed specialty on the first Eloísa import after an empty day bootstrap', async () => {
    process.env.HHR_SPECIALTY_EPISODE_ASSIGNMENT = 'enabled';
    const base = makeRecord();
    const metadata = {
      schemaVersion: 3,
    ...
    const imported = {
      ...remote,
      beds: {
        R2: { ...base.beds.R1, bedId: 'R2', specialty: 'Med Interna' },
      },
    };
    const { admin, docRef, set } = createAdminMock({
      remoteData: remote,
      previousData,
      specialtyPolicyData: {
        schemaVersion: 1,
    ...
    const response = await api.saveDailyRecordWithClinicalAuthority.run(
      {
        date: base.date,
        ...
        syncContract: {
          ...
          baseRevision: 2,
          changedPaths: ['*'],
          mutationId: 'first-eloisa-import',
        },
      },
      makeContext()
    );
    ...
    expect(set).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        beds: expect.objectContaining({
          R2: expect.objectContaining({ specialty: 'Cirugía', specialtyAssignment: metadata }),
        }),
      })
    );
  });

  it('does not read yesterday for an episode whose name and RUT are temporarily blank', async () => {
    process.env.HHR_SPECIALTY_EPISODE_ASSIGNMENT = 'enabled';
    const base = makeRecord();
    const occupant = {
      ...base.beds.R1,
    ...
    const remote = { ...base, meta: { revision: 2 }, beds: { R1: occupant } };
    const imported = { ...remote, beds: { R1: { ...occupant, age: '42' } } };
    const { admin, transaction } = createAdminMock({
      remoteData: remote,
      previousData: { ...base, date: '2026-05-12' },
    });
    const api = createDailyRecordWriteAuthorityFunctions({
      firestore: admin.firestore(),
      Timestamp: admin.firestore.Timestamp,
    ...
    const response = await api.saveDailyRecordWithClinicalAuthority.run(
      {
        ...
        syncContract: {
          ...
          baseRevision: 2,
          changedPaths: ['*'],
          mutationId: 'ordinary-episode-update',
        },
      },
      makeContext()
    );
    expect(response.success).toBe(true);
    expect(
      transaction.get.mock.calls.some(
        ([ref]) => (ref as { path?: string }).path === 'previous-daily-record-doc'
      )
    ).toBe(false);
  });
  ...
  it('persists verified provenance copied by a partial bed swap', async () => {
    ...
    const patch = {
      'beds.R1.patientName': 'Paciente Dos',
      'beds.R1.rut': '22.222.222-2',
      'beds.R1.clinicalEpisodeId': 'ep-dos',
      'beds.R1.specialty': '',
      'beds.R1.specialtyAssignment': null,
      'beds.R2.patientName': 'Paciente Uno',
      'beds.R2.rut': '11.111.111-1',
      'beds.R2.clinicalEpisodeId': 'ep-uno',
      'beds.R2.specialty': 'Cirugía',
    };
    const { admin, docRef, update, set } = createAdminMock({
      remoteData: remote,
      policyData: { schemaVersion: 2, clinicalBatchMode: 'enforced' },
      specialtyPolicyData: {
        schemaVersion: 1,
  ...
  it('accepts one fresh Jev receipt as one authorized patient write plus audit', async () => {
    ...
    const specialtyPolicyData = {
      ...
      aiRubrics: Object.fromEntries(
        OPTIONS.map((key: string) => [key, `Criterio de prueba sintético para ${key}.`])
      ),
    };
    const evidence = buildJevEvidence({
      ...
      target: 'bed',
      patient,
      policy: specialtyPolicyData,
    });
    const aiRequestData = {
      status: 'complete',
      requesterUid: 'synthetic-user',
      date,
      bedId: 'R1',
      target: 'bed',
      episodeId: 'ep-uno',
    ...
    const payload = {
      date,
      patch: { 'beds.R1.specialty': 'Med Interna' },
      syncContract: {
        mutationId: 'ai-mutation',
        baseRevision: 2,
        changedPaths: ['beds.R1.specialty'],
      },
      specialtyIntent: {
    ...
    await expect(
      functionsApi.patchDailyRecordWithClinicalAuthority.run(payload, context)
