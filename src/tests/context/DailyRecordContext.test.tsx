...
describe('DailyRecordContext', () => {
    const mockValue = {
        ...
        inventory: { occupiedCount: 1, blockedCount: 0, availableCount: 23, occupancyRate: 4, occupiedBeds: [], freeBeds: [], blockedBeds: [], isFull: false },
        stabilityRules: { isDateLocked: false, isDayShiftLocked: false, isNightShiftLocked: false, canEditField: () => true, canPerformActions: true },
        refresh: vi.fn()
    } as unknown as DailyRecordContextType;

    const TestComponent = ({ hook }: { hook: () => any }) => {
        const data = hook();
    ...
    it('should handle missing record in staffValue', () => {
        const valueWithoutRecord = { ...mockValue, record: null } as any;
        render(
    ...
    it('should provide default sync status when value is null', () => {
        render(
            <DailyRecordProvider value={null as any}>
                <TestComponent hook={useDailyRecordSync} />
            </DailyRecordProvider>
        );
