...
describe('networkUtils', () => {
  ...
  it('aborts immediately when shouldRetry returns false', async () => {
    const sentinel = new Error('no-retry');
    const fn = vi.fn().mockRejectedValue(sentinel);

    await expect(
      withRetry(fn, { maxRetries: 3, initialDelay: 1, shouldRetry: () => false })
    ).rejects.toBe(sentinel);

    // Called once — no retries because shouldRetry blocked them
    expect(fn).toHaveBeenCalledTimes(1);
  });

  it('still retries when shouldRetry returns true for that error', async () => {
    const fn = vi.fn().mockRejectedValueOnce(new Error('transient')).mockResolvedValue('ok');

    const result = await withRetry(fn, { initialDelay: 1, shouldRetry: () => true });
