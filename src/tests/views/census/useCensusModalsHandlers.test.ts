...
describe('useCensusModalsHandlers', () => {
  it('handles move/copy close and target updates', () => {
    ...
    const targetUpdater = setActionState.mock.calls[1][0] as (
      previous: ReturnType<typeof createInitialActionState>
    ) => ReturnType<typeof createInitialActionState>;
    expect(
      targetUpdater({
        type: 'move',
        sourceBedId: 'R1',
  ...
  it('handles discharge updates and ignores undefined target patches', () => {
    ...
    const statusUpdater = setDischargeState.mock.calls[0][0] as (
      previous: ReturnType<typeof createInitialDischargeState>
    ) => ReturnType<typeof createInitialDischargeState>;
    expect(statusUpdater(createInitialDischargeState()).status).toBe('Fallecido');

    const cribUpdater = setDischargeState.mock.calls[1][0] as (
      previous: ReturnType<typeof createInitialDischargeState>
    ) => ReturnType<typeof createInitialDischargeState>;
    expect(cribUpdater(createInitialDischargeState()).clinicalCribStatus).toBe('Vivo');

    const closeUpdater = setDischargeState.mock.calls[2][0] as (
      previous: ReturnType<typeof createInitialDischargeState>
    ) => ReturnType<typeof createInitialDischargeState>;
    expect(
      closeUpdater({
  ...
  it('handles transfer updates and close', () => {
    ...
    const updateUpdater = setTransferState.mock.calls[0][0] as (
      previous: ReturnType<typeof createInitialTransferState>
    ) => ReturnType<typeof createInitialTransferState>;
    expect(updateUpdater(createInitialTransferState()).transferEscort).toBe('TENS');

    const closeUpdater = setTransferState.mock.calls[1][0] as (
      previous: ReturnType<typeof createInitialTransferState>
    ) => ReturnType<typeof createInitialTransferState>;
    expect(
      closeUpdater({
  ...
  it('keeps handlers object reference stable when inputs do not change', () => {
    const setActionState = vi.fn();
    const setDischargeState = vi.fn();
    const setTransferState = vi.fn();
