 ...
 *
 * Owns the Centro HHR Turno and Scores surfaces. The host content script keeps
 * the shared shell, navigation, patient context, and clinical transition policy.
 */
(() => {
  'use strict';
  ...
  const create = dependencies => {
    ...
    if (
      ...
      throw new Error('No se pudo inicializar el Centro de Turno y Scores HHR.');
    }
  ...
  const renderScoresCenter = (root, encId) => {
    ...
    return Object.freeze({ renderHandoffCenter, renderScoresCenter });
  };

  globalThis.HhrHandoffScoresCenterRuntime = Object.freeze({ create });
})();
