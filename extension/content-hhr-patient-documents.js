  ...
  window.addEventListener('message', event => {
    ...
    if (data && data.type === 'HHR_RAYEN_CLINICAL_ACTION_REQUEST') {
      ...
      chrome.runtime.sendMessage(request).then(response => {
        chrome.runtime?.id && window.postMessage({ ...response, opened: response?.opened === true || (data.operation === 'prescription' && response?.ok === true && Number.isInteger(response.printTabId)), type: 'HHR_RAYEN_CLINICAL_ACTION_RESULT', reqId: data.reqId }, window.location.origin);
      }).catch(() => {
        chrome.runtime?.id && window.postMessage({ type: 'HHR_RAYEN_CLINICAL_ACTION_RESULT', reqId: data.reqId, ok: false, error: 'No se pudo conectar con la extensión Eloísa. Recárgala y vuelve a intentar.' }, window.location.origin);
      });
      return;
    }
    if (!data || data.type !== 'HHR_RAYEN_PATIENT_DOCUMENT_OPEN_REQUEST') return;
    const postDocument = response => chrome.runtime?.id && window.postMessage({ type: 'HHR_RAYEN_PATIENT_DOCUMENT_OPEN_RESULT', reqId: data.reqId, ok: response?.ok === true, opened: response?.opened === true, error: response?.error }, window.location.origin);
