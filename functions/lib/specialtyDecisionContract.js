...
const SPECIALTIES = new Set([
  ...
  'Psiquiatría', 'Pediatría', 'Odontología', 'Otro',
]);
...
const parseSpecialtyIntent = value => {
  ...
  if (!object(value) || !['manual', 'accept_ai'].includes(value.kind) || !TARGETS.includes(value.target)) {
  ...
  if (!bedId || bedId.includes('.') || !episodeId || episodeId.length > 160 ||
  ...
  if (value.kind === 'accept_ai' &&
      (typeof value.requestId !== 'string' || !/^[A-Za-z0-9_-]{12,100}$/.test(value.requestId) ||
       !specialty || specialty === 'Otro')) {
    throw new SpecialtyDecisionError('invalid-argument', 'Invalid Jev acceptance.');
  }
  return { kind: value.kind, bedId, target: value.target, episodeId,
    value: specialty, expectedDecisionId: value.expectedDecisionId,
...
const protectSpecialtyDecisions = ({ remoteRecord, priorRecord, candidate, intent, patch, actorUid, mutationId, now, aiDecision, guardScalarChanges = true }) => {
  ...
  eachTarget(candidate, ({ bedId, target, patient }) => {
    ...
    const scalarPath = target === 'clinicalCrib'
      ? `beds.${bedId}.clinicalCrib.specialty` : `beds.${bedId}.specialty`;
    ...
    if (isIntentTarget) {
      ...
      if (!patch || !Object.prototype.hasOwnProperty.call(patch, scalarPath) ||
          ...
          !/^[A-Za-z0-9_-]{1,120}$/.test(text(mutationId)) || !text(actorUid)) {
        throw new SpecialtyDecisionError('invalid-argument', 'Specialty intent does not match patch.');
      }
      const currentMeta = validExistingMeta(remote.specialtyAssignment, remote)
        ? remote.specialtyAssignment : null;
    ...
    if (sameEpisode) {
      if (remote.specialtyAssignment !== undefined) {
        ...
        if (patient.specialtyAssignment !== undefined &&
            !sameMeta(patient.specialtyAssignment, remote.specialtyAssignment)) {
          throw new SpecialtyDecisionError('failed-precondition', 'Specialty metadata is server-owned.');
        }
      ...
      if (scalarProtected && text(patient.specialty) !== text(remote.specialty)) {
        throw new SpecialtyDecisionError('failed-precondition', 'Specialty change requires explicit intent.');
      }
    ...
    if (episodeId) {
      for (const sourceRecord of [remoteRecord, priorRecord]) {
        eachTarget(sourceRecord, ({ patient: previous }) => {
          if (!prior && text(previous.clinicalEpisodeId) === episodeId) prior = previous;
        });
      }
    }
    if (patient.specialtyAssignment != null) {
      if (!prior || !validExistingMeta(prior.specialtyAssignment, prior) ||
          !sameMeta(patient.specialtyAssignment, prior.specialtyAssignment) ||
          text(patient.specialty) !== text(prior.specialty)) {
        throw new SpecialtyDecisionError('failed-precondition', 'Specialty metadata requires a confirmed episode.');
      }
      return;
    }
    if (prior?.specialtyAssignment != null) {
      if (!validExistingMeta(prior.specialtyAssignment, prior) ||
          text(patient.specialty) !== text(prior.specialty)) {
...
module.exports = { SPECIALTIES, SpecialtyDecisionError, parseSpecialtyIntent,
  protectSpecialtyDecisions, getPatient, validExistingMeta };
