...
| `GOOGLE-HANDOFF-SHEET` | `MEDICAL_HANDOFF_APPS_SCRIPT_URL`, `MEDICAL_HANDOFF_SHARED_SECRET` (Firebase) y `HHR_HANDOFF_SHARED_SECRET` (Apps Script) | Firebase Secret Manager + Apps Script    | `openMedicalHandoffSpreadsheet`             | Alto — permite solicitar la creación o actualización de planillas con datos clínicos          | Anual, ante incidente o cambio de cuenta institucional                       |
...
1. Generar un secreto nuevo con un CSPRNG y de al menos 24 caracteres. No registrarlo,
   commitearlo ni incluirlo como argumento de shell.
2. Introducir el valor mediante un canal seguro en `HHR_HANDOFF_SHARED_SECRET`, dentro de las
   propiedades del Apps Script institucional.
3. Introducir el mismo valor mediante la entrada segura de Firebase CLI con
...
6. Si el smoke test falla, restaurar el valor anterior compatible tanto en
   `HHR_HANDOFF_SHARED_SECRET` como en `MEDICAL_HANDOFF_SHARED_SECRET`, volver a desplegar la
   función y repetir el smoke test. No eliminar ningún despliegue durante este rollback.
7. Eliminar despliegues antiguos de Apps Script sólo después de que el smoke test confirme la
   integración nueva.
